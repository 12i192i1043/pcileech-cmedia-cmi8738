...
module pcileech_pcie_tlp_a7(
    ...
    // Step 4: BAR controller exports channel DMA state to the fake DMA gen.
    wire        fake_ch0_running;
    wire [31:0] fake_ch0_dma_base;
    wire [15:0] fake_ch0_dma_size;
    wire        fake_ch1_running;
    wire [31:0] fake_ch1_dma_base;
    wire [15:0] fake_ch1_dma_size;
    // Step 4 SAFETY: driver-programmed flags + global enable
    wire        fake_ch0_frame1_written;
    wire        fake_ch1_frame1_written;
    wire        fake_dma_enable;
    ...
    pcileech_tlps128_bar_controller i_pcileech_tlps128_bar_controller(
        ...
        .mem_wr_out       ( mem_wr_root.source    ),
        .mem_rd_out       ( mem_rd_root.source            ),
        // Step 4 DMA channel status exports
    ...
    // ------------------------------------------------------------------------
    // Step 4: fake DMA MRd generator + wrap mux on tlps_in6.
    // The wrap mux multiplexes tlps_static (pcileech injection) and the
    // fake DMA gen into a single slot, giving priority to tlps_static.
    // Fake DMA only fires when the emulated CMI8738 channels are running.
    // ------------------------------------------------------------------------
    IfAXIS128 tlps_fake_dma();
    IfAXIS128 tlps_in6_wrap();

    pcileech_fake_dma_gen i_pcileech_fake_dma_gen(
        .rst            ( rst                           ),
        .clk_pcie       ( clk_pcie                      ),
        .pcie_id        ( pcie_id                       ),
        .ch0_running    ( fake_ch0_running              ),
        .ch0_dma_base   ( fake_ch0_dma_base             ),
        .ch0_dma_size   ( fake_ch0_dma_size             ),
        .ch1_running    ( fake_ch1_running              ),
        .ch1_dma_base   ( fake_ch1_dma_base             ),
        .ch1_dma_size   ( fake_ch1_dma_size             ),
        // Step 4 SAFETY inputs
        .ch0_frame1_written ( fake_ch0_frame1_written   ),
        .ch1_frame1_written ( fake_ch1_frame1_written   ),
        .fake_dma_enable    ( fake_dma_enable           ),
        .tlps_out       ( tlps_fake_dma.source          )
    );

    pcileech_tlp_tx_wrap i_pcileech_tlp_tx_wrap(
        .rst            ( rst                           ),
        .clk_pcie       ( clk_pcie                      ),
        .orig_in        ( tlps_static                   ),
        .fake_in        ( tlps_fake_dma.sink            ),
        .tlps_out       ( tlps_in6_wrap.source          )
    );
...
module pcileech_tlps128_filter(
    ...
    // Step 4: drop fake-DMA completions (tag 0xFC-0xFF) before they reach the
    //         pcileech host FIFO. host_mem_read_2 uses 0x00-0xFB so this does
    //         not affect any real pcileech traffic.
    // Tag is at AXIS tdata[79:72]; top-6-bits == 6'b111111 matches 0xFC-0xFF.
    wire is_fake_cpl = is_tlphdr_cpl && (tlps_in.tdata[79:74] == 6'b111111);
    wire filter_next = (filter && !first) || (cfgtlp_filter && first && is_tlphdr_cfg) || (alltlp_filter && first && !is_tlphdr_cpl && !is_tlphdr_cfg) || is_fake_cpl;
